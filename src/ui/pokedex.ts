...
export function openPokedex() {
  ...
  function renderGrid() {
    grid.innerHTML = '';
    const searchTerm = (searchEl as HTMLInputElement)?.value.toLowerCase().trim() || '';
    const genVal = (genFilter as HTMLSelectElement)?.value || 'all';
    const statusVal = (statusFilter as HTMLSelectElement)?.value || 'all';

    let visible = 0;
    POKEDEX_ALL.forEach((name, idx) => {
      // Use real PokeAPI ID from pokedexData — this ensures correct artwork and numbers
      // even when form variants are skipped from the list
      const pokeapiId = pokedexData[name]?.pokeapiId || (idx + 1);

      // Generation filter (based on real PokeAPI ID)
      if (genVal !== 'all') {
...
// PokeAPI form-name mapping: species names that need a form-specific /pokemon/ endpoint
...
/** Fetch pokemon data from PokeAPI, with fallback for form species */
async function fetchPokemonData(speciesName) {
  // Try the form-specific name first
  const pokeName = FORM_TO_POKEMON_MAP[speciesName] || speciesName;
  try {
    return await fetchPokeAPI(`pokemon/${pokeName}`);
  } catch (e) {
    // Fallback: try the species endpoint for form pokemon
    if (pokeName !== speciesName) {
      try { return await fetchPokeAPI(`pokemon/${speciesName}`); } catch (_) {}
    }
    throw e;
  }
}
